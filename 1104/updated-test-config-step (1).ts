...
export const TestConfigStep: React.FC<TestConfigStepProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Transform sonar data to use simplified domain names
  ...
  const isTeamConfigComplete = (project: string, team: string): boolean => {
    ...
    if (!config) return true; // If no config exists, it's considered complete
    ...
    // Only return false if there are actual incomplete configurations
  ...
  // Filter Sonar domains and projects based on search
  const filteredSonarDomains = sonarData.filter(
    domain => domain.sonarDomain.toLowerCase().includes(sonarDomainSearchText.toLowerCase())
  );
                                ...
                                return (
                                  <FormControlLabel
                                    ...
                                    control={
                                      <Checkbox
                                        checked={isChecked}
                                        onChange={() => handleSonarProjectToggle(currentProject, team, activeSonarDomain, proj.project)}
                                      />
                                    }
                                    label={proj.project}
                                  />
