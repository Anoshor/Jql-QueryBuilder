...
export const ReviewStep: React.FC<ReviewStepProps> = ({
  ...
  const getPathDisplayName = (path: string): string => {
    ...
    const project = testItData.find(p => p.Project_Id === projectId);
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      const suite = findSuite(currentSuites, suiteId);
      
      if (suite) {
        displayName += ` > ${suite.Suite_Name}`;
                  ...
                  {config?.testItPaths && config.testItPaths.length > 0 && (
                    <Box sx={{ ml: 2, mb: 1 }}>
                      ...
                      {Object.entries(groupedPaths).map(([projectId, paths]) => {
                        const project = testItData.find(p => p.Project_Id === projectId);
                        const projectName = project ? project.Project_Name : projectId;
