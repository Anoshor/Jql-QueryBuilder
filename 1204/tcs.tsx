...
export const TestConfigStep: React.FC<TestConfigStepProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Transform sonar data to use simplified domain names (example usage)
  ...
  const isTeamConfigComplete = (project: string, team: string): boolean => {
    ...
    if (!config) return true; // If no config exists, consider it complete for now
    
    // Check if there are any incomplete configurations
    const hasIncompleteTestIt = config.testItPaths.length === 0;
    const hasIncompleteSonar = config.sonarQubeProjects.some(
      (s) => s.sonarProjects.length === 0
    );
    
    // If either is incomplete, the entire team config is incomplete
    return !(hasIncompleteTestIt || hasIncompleteSonar);
  };

  // Handle TestIt paths change
  const handleTestItPathsChange = (project: string, team: string, paths: string[]) => {
    ...
    onTeamConfigChange(project, team, {
      ...currentConfig,
      testItPaths: paths,
    });
  };

  // Toggle entire sonar domain
  const handleSonarDomainToggle = (project: string, team: string, sonarDomain: string) => {
    ...
    if (existingDomain) {
      ...
      // add it
      newSonarProjects = [
        ...currentConfig.sonarQubeProjects,
        { sonarDomain, sonarProjects: [] },
      ];
  ...
  // Toggle individual sonar project
  const handleSonarProjectToggle = (
    project: string,
    team: string,
    sonarDomain: string,
    sonarProject: string
    ...
    const currentConfig = teamConfigs[project]?.[team] || { testItPaths: [], sonarQubeProjects: [] };
    const domainIndex = currentConfig.sonarQubeProjects.findIndex(
      (s) => s.sonarDomain === sonarDomain
    );
  ...
  // Select / Deselect all projects in a domain
  ...
  // Filter Sonar domains by search
  ...
  // Filter Sonar projects by domain + search
  ...
  // Expand/collapse team accordion
      ...
      >
        {selectedProjects.map((project, index) => {
          ...
          const configuredCount =
            projectTeams[project.key]?.filter((team) =>
            ...
            <Tab
              ...
              label={
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Badge 
                    badgeContent={`${configuredCount}/${teamsCount}`} 
                    color="primary" 
                    sx={{ ml: 1 }}
                  />
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    {/* Our new multi-level TestItTreeView */}
                    <TestItTreeView
                      testItData={testItData}
                      selectedPaths={teamConfig.testItPaths}
                      onPathsChange={(paths) =>
                        handleTestItPathsChange(currentProject, team, paths)
                      }
                    />
                  </Box>
                </Grid>
                ...
                <Grid item xs={12}>
                  ...
                  <Grid container spacing={3}>
                    {/* Left Side - Sonar Domains */}
                    <Grid item xs={6}>
                      <Typography variant="subtitle2" gutterBottom>
                        Sonar Domains
                      </Typography>
                      {/* Search Sonar Domains */}
                      <TextField
                        fullWidth
                        size="small"
                        ...
                        <Box sx={{ mb: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                          {teamConfig.sonarQubeProjects.map((config) => (
                            <Chip
                              ...
                              onDelete={() =>
                                handleSonarDomainToggle(currentProject, team, config.sonarDomain)
                              }
                              color={
                                activeSonarDomain === config.sonarDomain ? 'primary' : 'default'
                              }
                              variant={
                                activeSonarDomain === config.sonarDomain ? 'filled' : 'outlined'
                              }
                              size="small"
                            />
                          ))}
                      ...
                      <Paper sx={{ maxHeight: 300, overflow: 'auto', p: 2 }}>
                        <FormGroup>
                          {filteredSonarDomains.map((domain) => (
                            ...
                            >
                              <Checkbox
                                checked={teamConfig.sonarQubeProjects.some(
                                  (s) => s.sonarDomain === domain.sonarDomain
                                )}
                                onChange={() =>
                                  handleSonarDomainToggle(currentProject, team, domain.sonarDomain)
                                }
                                onClick={(e) => e.stopPropagation()}
                              />
                              <Box
                                sx={{ ml: 1 }}
                                onClick={() => handleSetActiveSonarDomain(domain.sonarDomain)}
                              >
                    ...
                    {/* Right Side - Sonar Projects */}
                    <Grid item xs={5.5}>
                      <Typography variant="subtitle2" gutterBottom>
                        {activeSonarDomain
                          ? `Projects for "${activeSonarDomain}"`
                          : 'Select a Sonar domain to view projects'}
                      </Typography>
                      {activeSonarDomain && (
                        <>
                            ...
                            InputProps={{
                              ...
                              endAdornment: sonarProjectSearchText && (
                                <InputAdornment position="end">
                                  <IconButton
                                    size="small"
                                    onClick={() => setSonarProjectSearchText('')}
                                  >
                                    <ClearIcon />
                                  </IconButton>
                                </InputAdornment>
                          ...
                          />
                          {/* Selected Sonar Projects */}
                          {(() => {
                            const activeConfig = teamConfig.sonarQubeProjects.find(
                              (s) => s.sonarDomain === activeSonarDomain
                            );
                          ...
                          <Box sx={{ mb: 2 }}>
                            <Button
                              ...
                              color="primary"
                              onClick={() =>
                                handleToggleAllSonarProjects(currentProject, team, activeSonarDomain)
                              }
                            >
                              {selectAllStatus[`${team}-${activeSonarDomain}`]
                                ? 'Deselect All'
                                : 'Select All'}
                            </Button>
                          </Box>
                          ...
                          <Paper sx={{ maxHeight: 300, overflow: 'auto', p: 2 }}>
                            <FormGroup>
                              {getFilteredSonarProjects(activeSonarDomain).map((proj) => {
                                ...
                                const isChecked =
                                  activeConfig?.sonarProjects.includes(proj.project) || false;
