...
export const ReviewStep: React.FC<ReviewStepProps> = ({
  ...
  const totalMembers = Object.values(teamMembers).reduce(
    (total, projectMembers) =>
      total +
      Object.values(projectMembers).reduce((teamTotal, memberArray) => teamTotal + memberArray.length, 0),
    0
  );
  ...
  const getPathDisplayName = (path: string): string => {
    ...
    let currentSuites = project['Project_Test_Suites'][0]?.Suites || [];
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {selectedProjects.map(project => (
            <Chip key={project.key} label={`${project.name} (${project.key})`} />
          ))}
        </Box>
      </Box>
      <Box sx={{ mb: 3 }}>
            ...
            {projectTeams[project.key]?.map(team => (
              <Box key={team} sx={{ ml: 2, mb: 1 }}>
                <Typography variant="body1">
                  {team} <Box component="span" sx={{ ml: 1, color: 'text.secondary' }}>
        ...
        {selectedProjects.map(project => (
          <Box key={project.key} sx={{ mb: 2 }}>
            ...
            {projectTeams[project.key]?.map(team => {
              ...
              const groupedPaths = config?.testItPaths ? groupPathsByProject(config.testItPaths) : {};
              return (
                <Box key={team} sx={{ ml: 2, mb: 1 }}>
                  ...
                  {config?.testItPaths && config.testItPaths.length > 0 && (
                    <Box sx={{ ml: 2, mb: 1 }}>
                      ...
                      {Object.entries(groupedPaths).map(([projectId, paths]) => {
                        const proj = testItData.find(p => p['Project ID'] === projectId);
                        const projectName = proj ? proj['Project Name'] : projectId;
                        return (
                          <Box key={projectId} sx={{ ml: 2 }}>
                            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                              {projectName} <Box component="span" sx={{ ml: 1, color: 'text.secondary', fontWeight: 'normal' }}>
                                ({paths.length} paths)
                              </Box>
                            </Typography>
                            <Box sx={{ ml: 2 }}>
                      ...
                      {config.sonarQubeProjects.map(sonar => (
                        <Box key={sonar.sonarDomain} sx={{ ml: 2 }}>
                          <Typography variant="body2">
                            {sonar.sonarDomain} <Box component="span" sx={{ ml: 1, color: 'text.secondary' }}>
